...
function addMissingEndpoints(collection) {
  ...
  let schedulingSection = collection.item.find(item => item.name === 'Staff Scheduling');
  if (!schedulingSection) {
    ...
    collection.item.push(schedulingSection);
  }
  ...
  // Add leave allocation endpoints
  let leaveSection = collection.item.find(item => item.name === 'Staff Leave Management');
  if (!leaveSection) {
    leaveSection = {
    ...
    const authIndex = collection.item.findIndex(item => item.name === 'Staff Authentication');
    if (authIndex !== -1) {
      collection.item.splice(authIndex + 1, 0, leaveSection);
    } else {
      collection.item.push(leaveSection);
    }
  }
...
// Also update the main collection
