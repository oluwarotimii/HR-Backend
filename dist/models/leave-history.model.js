...
class LeaveHistoryModel {
    ...
    static async findByUserId(userId) {
        const [rows] = await pool.execute(`SELECT * FROM ${this.tableName} WHERE user_id = ? ORDER BY start_date DESC`, [userId]);
        return rows;
    }
    ...
    static async findByLeaveType(leaveTypeId) {
        const [rows] = await pool.execute(`SELECT * FROM ${this.tableName} WHERE leave_type_id = ? ORDER BY start_date DESC`, [leaveTypeId]);
        return rows;
    }
    static async create(historyData) {
        const [result] = await pool.execute(`INSERT INTO ${this.tableName} (user_id, leave_type_id, start_date, end_date, days_taken, reason, approved_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)`, [
            historyData.user_id,
    ...
    static async getTotalDaysTaken(userId, leaveTypeId, startDate, endDate) {
        const [rows] = await pool.execute(`SELECT SUM(days_taken) as total_days FROM ${this.tableName}
    ...
    static async hasOverlappingLeave(userId, startDate, endDate) {
        const [rows] = await pool.execute(`SELECT id FROM ${this.tableName}
