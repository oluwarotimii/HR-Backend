...
class LeaveAllocationModel {
    ...
    static async findByUserId(userId) {
        ...
        const cachedAllocations = await CacheService.get(cacheKey);
        if (cachedAllocations) {
            return cachedAllocations;
        }
        const [rows] = await pool.execute(`SELECT * FROM ${this.tableName} WHERE user_id = ? ORDER BY cycle_start_date DESC`, [userId]);
    ...
    static async findByCycleDates(startDate, endDate) {
        const [rows] = await pool.execute(`SELECT * FROM ${this.tableName} WHERE cycle_start_date >= ? AND cycle_end_date <= ?`, [startDate, endDate]);
        return rows;
    }
    static async create(allocationData, connection) {
        ...
        const hasOverlappingCycle = existingAllocations.some(allocation => allocation.cycle_start_date.getTime() === allocationData.cycle_start_date.getTime() &&
    ...
    static async getRemainingBalance(userId, leaveTypeId) {
        const [rows] = await pool.execute(`SELECT allocated_days, used_days, carried_over_days FROM ${this.tableName}
    ...
    static async bulkCreate(allocationsData) {
      ...
      VALUES ${placeholders.join(', ')}`;
        ...
        const [rows] = await pool.execute(`SELECT * FROM ${this.tableName} WHERE user_id IN (${inPlaceholders}) AND leave_type_id = ? ORDER BY created_at DESC`, selectValues);
        return rows;
    }
    static async createForAllUsers(leaveTypeId, allocatedDays, cycleStartDate, cycleEndDate, carriedOverDays = 0) {
        const [users] = await pool.execute(`SELECT id FROM users WHERE status = 'active'`);
