...
/**
 * One workbook, two worksheets — Summary (grouped by branch) and Leaderboard
 * (flat ranked list). Two conceptually different tables (one aggregate, one
 * a ranking) are clearer as separate sheets than crammed into one.
 */
export async function renderAttendanceReportExcel(data: AttendanceReportData): Promise<Buffer> {
